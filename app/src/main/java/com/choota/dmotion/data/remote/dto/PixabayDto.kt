...
import com.choota.dmotion.domain.model.PixImage
...
data class PixabayDto(
    ...
    val hits: List<PixImageItem?>? = null,
...
fun PixabayDto.toImages(): List<PixImage> {
    return hits!!.map {
        PixImage(it!!.webformatURL, it.userImageURL, it.tags, it.id)
    }
}

fun PixabayDto.toPage(): PixImagePage {
    return PixImagePage(
        this.toImages()
    )
}